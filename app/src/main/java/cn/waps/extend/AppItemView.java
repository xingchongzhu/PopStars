...
import cn.waps.extend.AppDetail.onAwardClickListener;

public class AppItemView{

	onAwardClickListener monAwardClickListener;
	public void setOnAwardClickListener(onAwardClickListener monAwardClickListener){
		this.monAwardClickListener = monAwardClickListener;
	}
	...
	public View getAdapterView(final Context context, final AdInfo adInfo,
		...
		GradientDrawable click_bg_grad = new GradientDrawable(
		...
		try {
			...
			if (itemWidth == 0 || itemHeight == 0) {
				itemWidth = ((Activity) context).getWindowManager()
			...
			whole_layout.setLayoutParams(new ListView.LayoutParams(itemWidth,
					itemHeight));

			r_layout = new RelativeLayout(context);

			int item_whole_bg_id = context.getResources().getIdentifier(
					"item_bg", "drawable", context.getPackageName());
			...
			int item_text_bg_id = context.getResources().getIdentifier(
					"item_text_bg", "drawable", context.getPackageName());
			...
			app_name.setId(2);
			app_name.setLayoutParams(new LinearLayout.LayoutParams(
			...
			// 金币奖励
			TextView award = new TextView(context);
			award.setText(context.getString(R.string.awardPoints));
			award.setTextSize(12);
			award.setTextColor(Color.GRAY);
			award.setPadding(5, 0, 5, 0);
			RelativeLayout.LayoutParams params5 = new RelativeLayout.LayoutParams(
					LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
			params5.addRule(RelativeLayout.ALIGN_TOP, app_name.getId());
			params5.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
			title_size_layout.addView(award,params5);
			...
			down_img_layout.setLayoutParams(new LinearLayout.LayoutParams(
					down_layout_width, LayoutParams.FILL_PARENT));

			ImageView downImage = new ImageView(context);
			int down_bg_id = context.getResources().getIdentifier("down_ico",
			...
			d_layout.setLayoutParams(new RelativeLayout.LayoutParams(
					LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
			...
			RelativeLayout.LayoutParams down_img_params = new RelativeLayout.LayoutParams(
					LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
			down_img_params.addRule(RelativeLayout.CENTER_IN_PARENT);

			down_img_layout.addView(d_layout);
			// down_img_layout.addView(award);
			...
			line_layout_2.setLayoutParams(new LinearLayout.LayoutParams(
					line_size, LayoutParams.FILL_PARENT));
			...
			down_img_layout.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(monAwardClickListener != null)
						monAwardClickListener.updatAward(10);
				}
			...
			// int width =
			// ((Activity)context).getWindowManager().getDefaultDisplay().getWidth()-83;
			// 点击效果的半透明遮罩布局
			final LinearLayout l_layout = new LinearLayout(context);
			l_layout.setLayoutParams(new RelativeLayout.LayoutParams(
					LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
